...
// This file centralizes product data to avoid circular dependencies and compilation issues.

// The Product interface, moved from [productId]/page.tsx
...
export const placeholderProducts: Product[] = rawProductsData.flat().map((p: any, index: number) => {
  const originalTags = p.tagsFromInput?.map((t: string) => t.toLowerCase()) || [];
  ...
  (p.tagsFromInput || []).forEach((tag: string) => {
